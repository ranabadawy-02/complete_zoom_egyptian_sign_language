...
def get_peer_state(peer_id):
    if peer_id not in peer_states:
        peer_states[peer_id] = {
            "frames_buffer":         deque(maxlen=120),
...
MODEL_PATH = 'best_18_sentence_mstp(2).pth'
model      = None
vocab      = None
idx2gloss  = None
holistic   = None
...
# ==============================================================================
# THRESHOLDS — identical to sentence.py
# ==============================================================================
CONFIDENCE_THRESHOLD = 0.35
MOTION_THRESHOLD     = 0.012
REST_FRAMES_REQUIRED = 5
MIN_SIGN_DURATION    = 8
...
def process_frame():
    ...
    # ------------------------------------------------------------------
    # Decode frame — NO flip, NO resize
    ...
    if state["prev_hand_positions"] and current_hand_positions:
        ...
        state["motion_buffer"].append(avg_movement)
    ...
    if state["is_moving"]:
        ...
        # Normal end-of-sign after rest frames
        elif (state["is_currently_signing"]
                and state["rest_counter"] >= REST_FRAMES_REQUIRED):
            state["is_currently_signing"] = False
